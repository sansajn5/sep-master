...
const start = async (): Promise<void> => {
  ...
  logger.info('ClientService connected on database');
  ...
  }).unless({ path: ['/api/auth/register', '/api/auth/login', '/api/health'] });
  ...
  expressApp.post('/api/auth/login', (req: express.Request, res: express.Response) => {
    serviceLogger.fireLog(req.header(Constants.RequestId), 'info', 'Enter method POST /api/auth/login');
    return app.authorize(req.body).then((data) => {
  ...
  expressApp.get('/api/clients/:merchantId', (req: express.Request, res: express.Response) => {
    const { merchantId } = req.query; 
    serviceLogger.fireLog(req.header(Constants.RequestId), 'info', `Enter method /api/clients/:merchantId with ${merchantId}`);
    return app.getClientPaymentMethods(merchantId).then((data) => {
      res.status(200).json(data)
    }).catch((err) => {
      res.status(400).json(err);
    })
  ...
  const port = process.env.CLIENT_HTTP_PORT || 3000;
  expressApp.listen(port, async () => {
    logger.info(`ClientService is listening on port ${port}!`);
  });
}
start();